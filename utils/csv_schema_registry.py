"""CSV Schema Registry for CSV reader fields.

This extends the bash/python registry pattern used by network fields to the
full monitoring CSV layout.

Responsibilities:
1. Define each logical field: {logical_name, semantic_type, segment, provider_aware, physical_name_template}
2. Resolve logical fields to physical CSV column names for readers.
3. Validate core invariants such as timestamp first, segment order, and column count.

Provider-aware fields use stable logical names while their physical CSV names
can vary through provider rules. Readers should resolve logical names through
this registry instead of hard-coding physical column names.

Current scope: core sections such as disk, basic, and block are registered.
Dynamic sections are generated by their collectors. The bash implementation in
config/csv_schema_registry.sh must remain symmetric with this file.
"""
from __future__ import annotations
from typing import Dict, List, Optional, Iterable
from dataclasses import dataclass


# disk_field_prefix values. All providers currently use "normalized" physical
# names for provider-adjusted disk metrics. Provider identity is carried by the
# separate cloud_provider column, while the provider argument is retained for
# future provider-specific naming if needed. AWS/GCP numeric differences are
# handled by disk_converter before values are written to CSV.
DISK_FIELD_PREFIX = {
    "aws": "normalized",
    "gcp": "normalized",
    "other": "normalized",
}

VALID_SEMANTIC_TYPES = {
    "rate", "throughput", "iops", "latency", "queue", "utilization",
    "ratio", "io_size", "iops_provider_adjusted", "throughput_provider_adjusted",
    "timestamp", "gauge", "unknown",
}

VALID_SEGMENTS = {
    "basic", "device", "network", "ena", "overhead",
    "block", "qps", "execution", "node_cpu", "cgroup", "meta",
}


@dataclass(frozen=True)
class FieldDef:
    """Logical CSV field definition.

    logical_name: stable logical name used by readers.
    semantic_type: semantic category used by group_by_semantic.
    segment: CSV section name.
    provider_aware: whether the physical name is provider-dependent.
    physical_template: physical column template.
    """
    logical_name: str
    semantic_type: str
    segment: str
    provider_aware: bool
    physical_template: str


# CSV section order. Dynamic sections have runtime-dependent widths and are
# generated by their own header helpers. Static sections are registered with FieldDef.
SEGMENT_ORDER = ["basic", "device", "network", "ena", "overhead", "block", "qps", "execution", "node_cpu", "cgroup", "meta"]

# Dynamic sections are not represented as static FieldDef lists.
DYNAMIC_SEGMENTS = {"device", "ena"}


# Basic section fields.
# Source: generate_csv_header basic_header
# Physical names are fixed and match logical names.
_BASIC_FIELDS: List[FieldDef] = [
    FieldDef("timestamp",   "timestamp",   "basic", False, "timestamp"),
    FieldDef("cpu_usage",   "gauge",       "basic", False, "cpu_usage"),
    FieldDef("cpu_usr",     "gauge",       "basic", False, "cpu_usr"),
    FieldDef("cpu_sys",     "gauge",       "basic", False, "cpu_sys"),
    FieldDef("cpu_iowait",  "gauge",       "basic", False, "cpu_iowait"),
    FieldDef("cpu_soft",    "gauge",       "basic", False, "cpu_soft"),
    FieldDef("cpu_idle",    "gauge",       "basic", False, "cpu_idle"),
    FieldDef("mem_used",    "gauge",       "basic", False, "mem_used"),
    FieldDef("mem_total",   "gauge",       "basic", False, "mem_total"),
    FieldDef("mem_usage",   "gauge",       "basic", False, "mem_usage"),
]


# Disk section fields.
# Source: generate_device_header
# Only provider-adjusted fields are provider-aware; the rest use fixed physical names.
_DISK_FIELDS: List[FieldDef] = [
    FieldDef("disk_r_s",                       "rate",        "device", False, "{prefix}_r_s"),
    FieldDef("disk_w_s",                       "rate",        "device", False, "{prefix}_w_s"),
    FieldDef("disk_rkb_s",                     "throughput",  "device", False, "{prefix}_rkb_s"),
    FieldDef("disk_wkb_s",                     "throughput",  "device", False, "{prefix}_wkb_s"),
    FieldDef("disk_r_await",                   "latency",     "device", False, "{prefix}_r_await"),
    FieldDef("disk_w_await",                   "latency",     "device", False, "{prefix}_w_await"),
    FieldDef("disk_avg_await",                 "latency",     "device", False, "{prefix}_avg_await"),
    FieldDef("disk_aqu_sz",                    "queue",       "device", False, "{prefix}_aqu_sz"),
    FieldDef("disk_util",                      "utilization", "device", False, "{prefix}_util"),
    FieldDef("disk_rrqm_s",                    "rate",        "device", False, "{prefix}_rrqm_s"),
    FieldDef("disk_wrqm_s",                    "rate",        "device", False, "{prefix}_wrqm_s"),
    FieldDef("disk_rrqm_pct",                  "ratio",       "device", False, "{prefix}_rrqm_pct"),
    FieldDef("disk_wrqm_pct",                  "ratio",       "device", False, "{prefix}_wrqm_pct"),
    FieldDef("disk_rareq_sz",                  "io_size",     "device", False, "{prefix}_rareq_sz"),
    FieldDef("disk_wareq_sz",                  "io_size",     "device", False, "{prefix}_wareq_sz"),
    FieldDef("disk_total_iops",                "iops",        "device", False, "{prefix}_total_iops"),
    FieldDef("disk_iops_provider_adjusted",    "iops_provider_adjusted",       "device", True,  "{prefix}_{dfp}_iops"),
    FieldDef("disk_read_throughput_mibs",      "throughput",  "device", False, "{prefix}_read_throughput_mibs"),
    FieldDef("disk_write_throughput_mibs",     "throughput",  "device", False, "{prefix}_write_throughput_mibs"),
    FieldDef("disk_total_throughput_mibs",     "throughput",  "device", False, "{prefix}_total_throughput_mibs"),
    FieldDef("disk_throughput_provider_adjusted", "throughput_provider_adjusted", "device", True, "{prefix}_{dfp}_throughput_mibs"),
]

_BLOCK_FIELDS: List[FieldDef] = [
    FieldDef("local_block_height",      "gauge",   "block", False, "local_block_height"),
    FieldDef("mainnet_block_height",    "gauge",   "block", False, "mainnet_block_height"),
    FieldDef("block_height_diff",       "gauge",   "block", False, "block_height_diff"),
    FieldDef("local_health",            "gauge",   "block", False, "local_health"),
    FieldDef("mainnet_health",          "gauge",   "block", False, "mainnet_health"),
    FieldDef("data_loss",               "gauge",   "block", False, "data_loss"),
    FieldDef("sync_mode",               "unknown", "block", False, "sync_mode"),
    FieldDef("sync_status",             "unknown", "block", False, "sync_status"),
    FieldDef("lag_value",               "gauge",   "block", False, "lag_value"),
    FieldDef("lag_unit",                "unknown", "block", False, "lag_unit"),
    FieldDef("freshness_gap_seconds",   "gauge",   "block", False, "freshness_gap_seconds"),
    FieldDef("probe_error",             "unknown", "block", False, "probe_error"),
]

_EXECUTION_FIELDS: List[FieldDef] = [
    FieldDef("execution_mgas_per_sec",  "gauge",   "execution", False, "execution_mgas_per_sec"),
    FieldDef("execution_gas_per_sec",   "gauge",   "execution", False, "execution_gas_per_sec"),
    FieldDef("execution_metric_source", "unknown", "execution", False, "execution_metric_source"),
    FieldDef("execution_metric_status", "unknown", "execution", False, "execution_metric_status"),
]

_NODE_CPU_FIELDS: List[FieldDef] = [
    FieldDef("node_process_pid",                  "gauge",   "node_cpu", False, "node_process_pid"),
    FieldDef("node_process_cpu_pct",              "gauge",   "node_cpu", False, "node_process_cpu_pct"),
    FieldDef("node_thread_count",                 "gauge",   "node_cpu", False, "node_thread_count"),
    FieldDef("node_hottest_thread_tid",           "gauge",   "node_cpu", False, "node_hottest_thread_tid"),
    FieldDef("node_hottest_thread_name",          "unknown", "node_cpu", False, "node_hottest_thread_name"),
    FieldDef("node_hottest_thread_cpu_pct",       "gauge",   "node_cpu", False, "node_hottest_thread_cpu_pct"),
    FieldDef("node_hottest_thread_core",          "gauge",   "node_cpu", False, "node_hottest_thread_core"),
    FieldDef("node_top_threads_cpu_pct",          "unknown", "node_cpu", False, "node_top_threads_cpu_pct"),
    FieldDef("node_hottest_core_id",              "gauge",   "node_cpu", False, "node_hottest_core_id"),
    FieldDef("node_hottest_core_cpu_pct",         "gauge",   "node_cpu", False, "node_hottest_core_cpu_pct"),
    FieldDef("node_top_cores_cpu_pct",            "unknown", "node_cpu", False, "node_top_cores_cpu_pct"),
    FieldDef("node_cpu_concentration_top1_pct",   "gauge",   "node_cpu", False, "node_cpu_concentration_top1_pct"),
    FieldDef("node_cpu_concentration_top5_pct",   "gauge",   "node_cpu", False, "node_cpu_concentration_top5_pct"),
    FieldDef("node_cpu_status",                   "unknown", "node_cpu", False, "node_cpu_status"),
]

BLOCK_CACHE_REQUIRED_KEYS = [
    "timestamp",
    "local_block_height",
    "mainnet_block_height",
    "sync_mode",
    "sync_status",
]


class CSVSchemaRegistry:
    """CSV schema registry shared by readers and writers."""

    # Static fields in CSV segment order. Dynamic sections are generated elsewhere.
    _ALL_STATIC_FIELDS: List[FieldDef] = (
        _BASIC_FIELDS + _DISK_FIELDS + _BLOCK_FIELDS + _EXECUTION_FIELDS + _NODE_CPU_FIELDS
    )
    _FIELDS_BY_LOGICAL: Dict[str, FieldDef] = {f.logical_name: f for f in _ALL_STATIC_FIELDS}

    @classmethod
    def all_logical_names(cls) -> List[str]:
        return list(cls._FIELDS_BY_LOGICAL.keys())

    @classmethod
    def segment_logical_names(cls, segment: str) -> List[str]:
        """Return logical names for a registered static segment.

        Dynamic sections have no static FieldDef list and return [].
        """
        return [f.logical_name for f in cls._ALL_STATIC_FIELDS if f.segment == segment]

    @classmethod
    def segment_header(cls, segment: str, provider: str = "other",
                       device_prefix: str = "") -> str:
        """Generate a CSV header for a registered static segment.

        provider/device_prefix are relevant only for provider-aware fields.
        Dynamic sections return an empty string.
        """
        names = cls.segment_logical_names(segment)
        return ",".join(cls.resolve(ln, provider, device_prefix) for ln in names)

    @classmethod
    def get_field(cls, logical_name: str) -> Optional[FieldDef]:
        return cls._FIELDS_BY_LOGICAL.get(logical_name)

    @classmethod
    def get_semantic_type(cls, logical_name: str) -> str:
        f = cls._FIELDS_BY_LOGICAL.get(logical_name)
        return f.semantic_type if f else "unknown"

    @classmethod
    def resolve(cls, logical_name: str, provider: str, device_prefix: str) -> str:
        """Resolve a logical field name to a physical CSV column name.

        device_prefix is the logical device prefix, such as 'data',
        'accounts', or a complete runtime prefix such as 'data_nvme1n1'.
        provider selects the disk field prefix for provider-aware fields.

        Raises KeyError when logical_name is not registered.
        """
        f = cls._FIELDS_BY_LOGICAL.get(logical_name)
        if f is None:
            raise KeyError(
                f"unknown logical field: {logical_name!r} "
                f"(registered: {sorted(cls._FIELDS_BY_LOGICAL)})"
            )
        dfp = DISK_FIELD_PREFIX.get(provider, "normalized")
        return f.physical_template.format(prefix=device_prefix, dfp=dfp)

    @classmethod
    def group_by_semantic(cls, logical_names: Iterable[str]) -> Dict[str, List[str]]:
        groups: Dict[str, List[str]] = {}
        for ln in logical_names:
            groups.setdefault(cls.get_semantic_type(ln), []).append(ln)
        return groups

    @classmethod
    def provider_aware_fields(cls) -> List[str]:
        """Return all provider-aware logical field names."""
        return [f.logical_name for f in cls._FIELDS_BY_LOGICAL.values() if f.provider_aware]
