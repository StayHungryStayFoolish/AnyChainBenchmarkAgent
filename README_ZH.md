...
- 可以基于 artifact 解释报告图表，以及图表为空的原因。
- 可以为新增链、RPC method 和 weighted workload 生成插件化 onboarding plan。
...
# 解释每类报告图表需要哪些 CSV/字段，以及图表可用、为空或缺失的原因。
```
...
Agent 会检查已登记的 artifact，并解释 performance overview、CPU-disk correlation、
disk thresholds、per-method attribution、sync health 和 monitoring overhead。
