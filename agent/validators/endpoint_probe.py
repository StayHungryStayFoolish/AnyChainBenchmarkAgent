...
def _should_use_generic_jsonrpc_probe(
    ...
    if _chain_template_exists(chain):
        if adapter_family in {"jsonrpc", "evm", "ethereum", "ethereum_jsonrpc"} and _has_jsonrpc_custom_probe(chain, methods, method_params):
            return True
        return False
    if adapter_family in {"jsonrpc", "evm", "ethereum", "ethereum_jsonrpc"}:
        return True
...
def _validate_generic_jsonrpc_endpoint(
    ...
    *,
    ...
    if not selected_methods:
        ...
        return _finalize_result(result)
    health_method = "eth_chainId" if "eth_chainId" not in selected_methods else selected_methods[0]
...
def _call_request(request_data: dict[str, Any], timeout: float) -> tuple[int | str, str]:
    ...
    data = body.encode("utf-8") if isinstance(body, str) and body else body
...
def _adapter_family(chain: str) -> str:
    try:
        ...
        return ""
    meta = template.get("_meta", {}) if isinstance(template.get("_meta"), dict) else {}
    return str(meta.get("adapter_family") or "")
