...
def freeze_semantic_plan(
    ...
    *,
    ...
    for index, row in enumerate(normalized_units):
        ...
        required_verdict = str(row.get("required_unit_verdict") or "")
        if required_verdict not in {"", "context"}:
            raise ValueError("semantic plan unit verdict contract is invalid")
        if required_verdict == "context" and (
            str(row.get("disposition") or "") != "context" or owners
        ):
            raise ValueError(
                "semantic plan authoritative context unit is not context-only"
            )
...
def whole_plan_admission_prompt(semantic_policy: str) -> str:
    ...
    return (
        ...
        "grounded_arguments must contain exactly one row for every supplied required_value_grounding_argument and no other row. argument_name is the exact required_value_grounding_argument name copied verbatim, never an explanation or value. Its evidence_quote must be the shortest non-empty exact affirmative substring of one owned source unit's evidence_sources that semantically selects the exact immutable operation_arguments value; exclude contrast text and rejected alternatives from the quote. evidence_sources contains the immutable original source and, only for a previously unresolved atom, its Harness-bound user clarification. Merely naming the argument or dimension, asking to change it without selecting a value, stating a generic benchmark goal, or relying on workflow state does not ground a concrete value. Natural-language equivalents may ground a value only when they unambiguously select that exact value. For every argument listed in open_identity_grounding_arguments, a concrete value is grounded only by a named identity supplied by the user. A pronoun, deictic reference, generic category, quantifier, placeholder, or request for some other/another member of the category is not a named identity; reject the concrete selection so the registered typed intake can collect the name. Rejecting or excluding one value in a closed enum with multiple remaining values does not select any one remaining value; reject that concrete selection so the registered typed intake can ask the user. Actions with no required value-grounding arguments return an empty list. "
        ...
        "Each unit_verdict is {unit_id,verdict:'complete'|'support'|'context'|'unresolved'|'omitted',owner_action_ids:[string],evidence_quote:string,omitted_action_type:string,reason}. When a semantic unit supplies required_unit_verdict=context, copy that exact verdict and leave omitted_action_type empty; the independent relation authority already decided that this unit supports another unit and is not a separate demand, so this role is not open to reinterpretation. "
...
def _is_explicit_semantic_rejection(
    ...
    if any(
        str(row.get("verdict") or "") == "omitted"
        and not str(row.get("omitted_action_type") or "").strip()
        for row in rejected_units
    ):
        return False
    structural_errors = tuple(
        ...
        and not error.startswith(
            "whole-plan action unit is neither complete nor support:"
        )
        and not error.startswith(
            "whole-plan candidate retains unresolved unit:"
        )
    )
    if structural_errors:
        return False
    return bool(
        rejected_actions or rejected_units
    )
    ...
    *,
    ...
    for raw in action_rows:
        ...
        owned_sources = [
            ...
            for source in (
                (unit_records.get(unit_id) or {}).get("evidence_sources")
                or [
                    str(
                        (unit_records.get(unit_id) or {}).get(
                            "source_text"
                        )
                        or ""
                    )
    ...
    for raw in unit_rows:
        ...
        required_unit_verdict = str(
            record.get("required_unit_verdict") or ""
        )
        if required_unit_verdict and verdict != required_unit_verdict:
            errors.append(
                "whole-plan unit verdict contradicts its immutable verdict "
                f"contract: {unit_id}"
            )
        ...
        elif disposition == "context" and verdict not in {"context", "omitted"}:
