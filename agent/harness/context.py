            ...
            "entry_actions": [
                {
                    ...
                    "arguments": dict(spec.entry_intake_arguments),
                }
                for spec in ACTION_SPECS
...
def build_action_resolver_prompt() -> str:
    return (
        ...
        "When one turn selects exactly one declared option and an adjacent clause only explains the reason for that same selection, preserve the complete turn as one answer_pending transaction. Map the selection as direct evidence and the explanation as explanatory_context or operation_restatement support for that same action; do not turn the explanation into an unresolved sibling or a consultation. This applies to every numbered or yes/no option and every language. For example, 'choose skip; the endpoint needs no credential' is one pending answer, while 'choose skip; why would a credential be needed?' contains an independent question. A clause that asks a question, changes another value, selects another option, contradicts the selection, or requests another operation remains independently actionable and must not be absorbed as rationale. "
