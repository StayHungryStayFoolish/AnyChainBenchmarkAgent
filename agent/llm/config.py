...
def load_llm_config(env: Mapping[str, str] | None = None) -> LLMConfig:
    ...
    provider = source.get("LLM_PROVIDER", "vertex_gemini_openai").strip().lower()
    default_model = "gemini-2.5-pro" if provider == "vertex_gemini_openai" else "claude-3-7-sonnet@20250219"
    if provider == "openai":
