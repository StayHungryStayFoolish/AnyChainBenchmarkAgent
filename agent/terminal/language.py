...
_ZH = {
    ...
    "ask_detected_single_method": "链模板中 single RPC method 是 {method}。请选择：[1] 使用默认方法 / [2] 自定义 RPC method。",
    "ask_detected_mixed_weights": "链模板中 mixed 权重是 {weights}。请选择：[1] 使用默认权重 / [2] 自定义 RPC methods 和权重。",
    "ask_single_default_or_custom": "请选择 [1] 使用默认 single method，或 [2] 自定义 RPC method。",
    "ask_mixed_default_or_custom": "请选择 [1] 使用默认 mixed 权重，或 [2] 自定义 RPC methods 和权重。",
    ...
    "unsupported_chain_intro": "{chain} 不在当前 36 个 config/chains 模板中。当前支持链示例：{chains}。如果要新增链，我需要先判断它是否属于现有 adapter family。",
    "ask_unsupported_chain_family": "请选择协议/请求 family，或输入 family 名称：\n{families}",
    "invalid_family_choice": "无法识别这个 family 选择。",
    "unsupported_chain_family_recorded": "已记录 {chain} 属于 {family}。下一步需要 RPC method 证据。",
    "ask_unsupported_chain_methods": "请提供这个链要支持或压测的 RPC method 列表，用逗号分隔，例如 foo_getBalance,foo_getTransaction。",
    "unsupported_chain_onboarding_ready": "已生成 {chain} 的二次开发 handoff。它还不是已支持链，必须完成文档、fixture 和 smoke 验证后才能标记支持。",
    "unsupported_chain_new_family": "已判断为新/未知协议 family。不能直接改模板冒充支持，需要先生成新 family 设计与闭环验证计划。",
...
_EN = {
    ...
    "ask_detected_single_method": "The chain template single RPC method is {method}. Choose: [1] use the default method / [2] provide a custom RPC method.",
    "ask_detected_mixed_weights": "The chain template mixed weights are {weights}. Choose: [1] use default weights / [2] provide custom RPC methods and weights.",
    "ask_single_default_or_custom": "Choose [1] use the default single method, or [2] provide a custom RPC method.",
    "ask_mixed_default_or_custom": "Choose [1] use default mixed weights, or [2] provide custom RPC methods and weights.",
    ...
    "unsupported_chain_intro": "{chain} is not in the current 36 config/chains templates. Supported chain examples: {chains}. To add it, I first need to classify whether it fits an existing adapter family.",
    "ask_unsupported_chain_family": "Choose the protocol/request family, or type the family name:\n{families}",
    "invalid_family_choice": "I could not recognize that family choice.",
    "unsupported_chain_family_recorded": "Recorded {chain} as {family}. Next I need the RPC methods to support or benchmark.",
    "ask_unsupported_chain_methods": "Provide the RPC methods for this chain, separated by commas, for example foo_getBalance,foo_getTransaction.",
    "unsupported_chain_onboarding_ready": "Generated a secondary-development handoff for {chain}. It is not supported until docs, fixtures, and smoke validation are complete.",
    "unsupported_chain_new_family": "Classified as a new/unknown protocol family. Do not fake support with only a template; write a new-family design and closed-loop validation plan first.",
